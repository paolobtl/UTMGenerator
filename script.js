  ...
  const form = document.forms["utm-form"];
  const messageBox = document.querySelector("#utm-form > .message-box");
  const resultsBox = document.querySelector("#utm-result");
  const inputElements = document.querySelectorAll("input");

  // Functions
  const printMessage = (input, messageBox) => {
    resultsBox.textContent = "";
    messageBox.textContent = "";
    if (input.id === "url") {
      messageBox.textContent = `L'URL del sito web fornito non è un URL valido.`;
    } else if (input.id.includes("utm")) {
      messageBox.textContent =
        "Compila i campi obbligatori qui sopra e verrà generato un URL automaticamente.";
    }
  };
  const validateInputs = (requiredInputElements) => {
    messageBox.textContent = "";
    let count = 0;
    for (let input of requiredInputElements) {
      if (!!input.value.trim() && input.validity.valid) {
        input.ariaInvalid = false;
        count++;
      } else if (!input.validity.valid) {
        printMessage(input, messageBox);
        input.ariaInvalid = true;
      }
    }
    return requiredInputElements.length == count;
  };
  const generateURL = (inputElements) => {
    let utmElements = [];
    const utmURL = new URL(form["url"].value);

    for (let input of inputElements) {
      if (input.id !== 'url' && input.value) {
        utmElements.push(`${input.id.replace('-','_')}=${input.value}`);
      }
    }
    utmURL.search = decodeURIComponent(utmElements.join("&"));
  ...
  inputElements.forEach((input) =>
    input.addEventListener("input", () => {
      if (validateInputs(requiredInputElements)) {
        resultsBox.innerHTML = `${generateURL(inputElements)}
        <span class="tooltip-text">Copiato!</span>
         <svg
        fill="#000000"
        version="1.1"
        id="clipboard"
      ...
      >
        <g>
          <g>
            <g>
              <path
                d="M477.884,79.917h-50.825c-11.908-7.781-24.874-13.455-36.857-17.557c-5.595-14.695-14.537-27.921-26.047-38.28
              ...
              />
              <path
                d="M477.884,606.816H128.933c-32.334,0-58.64-26.306-58.64-58.641V138.058c0-32.334,26.306-58.64,58.64-58.64h50.676
            ...
            <g>
              <path
                d="M303.408,109.265c11.941,0,21.622-9.68,21.622-21.622c0-2.723-0.509-5.326-1.427-7.726
              ...
              />
              <path
                d="M303.408,109.765c-12.197,0-22.121-9.924-22.123-22.122c0-2.713,0.491-5.373,1.46-7.904
            ...
            <g>
              <path
                d="M187.724,278.191H296.66c11.83,0,21.42-9.59,21.42-21.42c0-11.83-9.591-21.42-21.42-21.42H187.724
                   c-11.83,0-21.42,9.59-21.42,21.42C166.304,268.601,175.894,278.191,187.724,278.191z"
              />
              <path
                d="M296.66,278.691H187.724c-12.087,0-21.92-9.833-21.92-21.92s9.833-21.92,21.92-21.92H296.66
            ...
            <g>
              <path
                d="M419.094,350.871h-231.37c-11.83,0-21.42,9.59-21.42,21.42s9.59,21.42,21.42,21.42h231.37
                   c11.83,0,21.42-9.59,21.42-21.42S430.924,350.871,419.094,350.871z"
              />
              <path
                d="M419.094,394.211h-231.37c-12.087,0-21.92-9.833-21.92-21.92s9.833-21.92,21.92-21.92h231.37
            ...
            <g>
              <path
                d="M352.891,466.855H187.724c-11.83,0-21.42,9.59-21.42,21.42s9.59,21.42,21.42,21.42h165.167
                   c11.83,0,21.42-9.59,21.42-21.42S364.721,466.855,352.891,466.855z"
              />
              <path
                d="M352.891,510.195H187.724c-12.087,0-21.92-9.833-21.92-21.92s9.833-21.92,21.92-21.92h165.167
        ...
        document.getElementById("clipboard").addEventListener("click", () => {
          copyContent(resultsBox.innerText.trim());
